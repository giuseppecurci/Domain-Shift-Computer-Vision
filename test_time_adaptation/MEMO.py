...
def get_best_augmentations(probabilities, top_k):
    """
    Takes a tensor of probabilities with dimension [num_augmentations,200] and outputs a tensor containing the probabilities
    corresponding to the augmentations with the lowest entropy of dimension [top_k, 200].
    ----------
    top_k: number of augmentations to select
    probabilities: a tensor of dimension [num_augmentations,200] 
    """
    entropies = torch.tensor([compute_entropy(prob) for prob in probabilities])
